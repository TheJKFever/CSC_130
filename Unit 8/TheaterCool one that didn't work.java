...
{	boolean tryAgain=false;
	...
	do {System.out.println("Please choose the price that you are willing to pay?");
		...
		String amountString = keyboard.next();//takes in dollar amount
		if (amountString.charAt(0)=='$')
			amountString = amountString.substring(1);
		amountString = amountString.trim();
		dollar = Integer.parseInt(amountString);//turns dollar amount into integer
...
{do {System.out.println("Please type in the Row and Seat number you would like to purchase?");
	rowToPurchase = (keyboard.nextInt()-1);
	seatToPurchase = (keyboard.nextInt()-1);
	if ((rowToPurchase>=(TheaterSeatingChart.length))||(seatToPurchase>=(TheaterSeatingChart[0].length)))
		System.out.println("That is not a valid seat, please choose between rows 1-9, and seats 1-10");
	} while ((rowToPurchase>=(TheaterSeatingChart.length))||(seatToPurchase>=(TheaterSeatingChart[0].length)));
}	
